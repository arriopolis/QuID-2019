...
def perform_tomography(obj, qubits, debug = False, initial_state = None, repetitions = 4096):
    n = len(qubits)
    ...
    if initial_state is None:
        res = cirq.Simulator().run(nmc, repetitions = repetitions).measurements
    else:
        ...
        ms = np.random.choice(np.arange(2**n), repetitions, p = np.square(np.abs(fs)))
    ...
    for i in range(n):
        ...
        if initial_state is None:
            res = cirq.Simulator().run(npmc, repetitions = repetitions).measurements
        else:
        ...
        if initial_state is None:
            res = cirq.Simulator().run(ipmc, repetitions = repetitions).measurements
        else:
